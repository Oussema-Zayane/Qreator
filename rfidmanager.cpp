...
void RFIDManager::processRFIDLine(const QString &line)
{
    ...
    if (uid.isEmpty()) {
        qDebug() << "❌ Impossible d'extraire l'UID de:" << originalLine;
        return;
    }

    if (uid.length() < 4) {
        qDebug() << "❌ UID trop court:" << uid << "(longueur:" << uid.length() << ")";
        return;
    }
    ...
    if (checkEmployeeUID(uid, nom, prenom, role)) {
        ...
        // Employé trouvé et actif
        qDebug() << "🎉 ACCÈS AUTORISÉ pour" << prenom << nom
                 << "(" << role << ") - ID:" << employeeId;

        // ⭐ ENVOYER "GRANTED" D'ABORD
        sendToArduino("GRANTED");

        // ⭐ ENSUITE ENVOYER LE NOM ET L'ID
        // Format: "Nom Prenom|ID"
        QString employeeInfo = prenom + " " + nom + "|" + employeeId;

        // Nettoyer le texte (retirer caractères spéciaux)
        employeeInfo = employeeInfo.simplified();

        // Envoyer avec un petit délai
        QTimer::singleShot(100, this, [this, employeeInfo]() {
            sendToArduino(employeeInfo);
            qDebug() << "📤 Infos employé envoyées:" << employeeInfo;
        });

        // Optionnel: Message de bienvenue court
        QString welcomeMsg = "BIENVENUE:" + prenom.left(10);
        QTimer::singleShot(200, this, [this, welcomeMsg]() {
            sendToArduino(welcomeMsg);
        });

    } else {
        // Employé non trouvé ou inactif
        qDebug() << "❌ ACCÈS REFUSÉ - Carte non autorisée";
        sendToArduino("DENIED");

        // ⭐ ENVOYER UN MESSAGE D'ERREUR AU LCD
        QTimer::singleShot(100, this, [this]() {
            sendToArduino("Carte Inconnue");
        });
    }
}
