...
#include <QGridLayout>
#include <QDockWidget>  // AJOUTEZ CETTE LIGNE
...
void MainWindow::refreshEquipementTable()
{
    ...
    QWidget *existingStats = findChild<QWidget*>("statsWidgetEquipements");
    if (!existingStats) {
        creerStatistiquesCirculairesEquipements();
    }
    mettreAJourStatistiquesCirculairesEquipements();
    // ==================================================
...
void MainWindow::afficherStatsEquipements()
{
    // 1. Récupérer les statistiques
    QMap<QString, int> stats = Equipement::getStatistiquesParEtat();

    // 2. Calculer le total
    int total = stats["En panne"] + stats["Neuf"] + stats["Occasion"];
    if (total == 0) total = 1;

    // 3. Calculer les pourcentages
    int pourcentPanne = (stats["En panne"] * 100) / total;
    int pourcentNeuf = (stats["Neuf"] * 100) / total;
    int pourcentOccasion = (stats["Occasion"] * 100) / total;

    // 4. TROUVER LA PAGE DES ÉQUIPEMENTS
    QWidget *pageEquipements = nullptr;

    // Essayez ces noms (vérifiez dans votre .ui)
    QStringList nomsPossibles = {
        "pageEquipements", "tabEquipements", "widgetEquipements",
        "equipementPage", "gestionEquipements", "Equipement"
    };

    for (const QString &nom : nomsPossibles) {
        pageEquipements = findChild<QWidget*>(nom);
        if (pageEquipements) {
            qDebug() << "Page équipements trouvée:" << nom;
            break;
        }
    }

    if (!pageEquipements) {
        qDebug() << "ERREUR: Page des équipements non trouvée!";
        return;
    }

    // 5. Chercher le frame des stats DANS LA PAGE DES ÉQUIPEMENTS
    QFrame *frameStats = pageEquipements->findChild<QFrame*>("frameStatsEquipements");

    if (!frameStats) {
        // Créer le frame DANS la page des équipements
        frameStats = new QFrame(pageEquipements);
        frameStats->setObjectName("frameStatsEquipements");

        // Style comme les clients
        frameStats->setStyleSheet(
            "QFrame {"
            "    background-color: #FFFFFF;"
        ...
        frameStats->setFixedHeight(120);

        // Layout pour les 3 cercles/statistiques
        QHBoxLayout *layoutStats = new QHBoxLayout(frameStats);
        layoutStats->setSpacing(20);
        layoutStats->setContentsMargins(15, 10, 15, 10);

        // Créer 3 widgets pour chaque statistique (comme les clients)
        QStringList categories = {"En panne", "Neuf", "Occasion"};
        ...
        for (int i = 0; i < 3; i++) {
            QWidget *widgetStat = new QWidget();
            widgetStat->setObjectName("widgetStat_" + categories[i].replace(" ", ""));
            widgetStat->setFixedSize(120, 100);

            QVBoxLayout *layoutStat = new QVBoxLayout(widgetStat);
            layoutStat->setAlignment(Qt::AlignCenter);
            layoutStat->setSpacing(5);

            // Label pour le pourcentage (comme un cercle)
            QLabel *labelPourcentage = new QLabel();
            labelPourcentage->setObjectName("labelPourcent_" + categories[i].replace(" ", ""));
            labelPourcentage->setAlignment(Qt::AlignCenter);
            labelPourcentage->setStyleSheet(QString(
                                                "QLabel {"
                                                "    font-size: 28px;"
                                                "    font-weight: bold;"
                                                "    color: %1;"
                                                ...
                                                "    max-height: 80px;"
                                                "}"
                                                ).arg(couleurs[i].name()).arg(couleurs[i].lighter(150).name()));

            // Label pour le texte
            QLabel *labelTexte = new QLabel(categories[i]);
            labelTexte->setObjectName("labelTexte_" + categories[i].replace(" ", ""));
            labelTexte->setAlignment(Qt::AlignCenter);
            labelTexte->setStyleSheet(
                "QLabel {"
                "    font-size: 12px;"
                "    color: #7F8C8D;"
                "    font-weight: bold;"
            ...
            layoutStat->addWidget(labelPourcentage);
            layoutStat->addWidget(labelTexte);

            layoutStats->addWidget(widgetStat, 0, Qt::AlignCenter);
        }

        // Ajouter le frame EN HAUT de la page des équipements
        QVBoxLayout *pageLayout = qobject_cast<QVBoxLayout*>(pageEquipements->layout());
        if (!pageLayout) {
            // Si pas de layout, en créer un
            pageLayout = new QVBoxLayout(pageEquipements);
            pageLayout->setContentsMargins(10, 10, 10, 10);
            pageLayout->setSpacing(10);
        }

        // Insérer le frame en première position (en haut)
        pageLayout->insertWidget(0, frameStats);
    }

    // 6. Mettre à jour les valeurs
    QStringList categories = {"Enpanne", "Neuf", "Occasion"};
    QList<int> pourcentages = {pourcentPanne, pourcentNeuf, pourcentOccasion};
    QList<int> valeurs = {stats["En panne"], stats["Neuf"], stats["Occasion"]};

    for (int i = 0; i < 3; i++) {
        QLabel *labelPourcent = findChild<QLabel*>("labelPourcent_" + categories[i]);
        QLabel *labelTexte = findChild<QLabel*>("labelTexte_" + categories[i]);

        if (labelPourcent) {
            labelPourcent->setText(QString("%1%").arg(pourcentages[i]));
        }

        if (labelTexte) {
            labelTexte->setText(QString("%1\n%2 équipements")
                                    .arg(categories[i].replace("Enpanne", "EN PANNE"))
                                    .arg(valeurs[i]));
        }
    }
}
...
void MainWindow::ajouterStatsEquipements()
{
    // 1. Récupérer les statistiques
    QMap<QString, int> stats = Equipement::getStatistiquesParEtat();
    ...
    // 4. Vérifier si les stats existent déjà
    QWidget *widgetStats = findChild<QWidget*>("widgetStatsEquipements");
    if (widgetStats) {
        // Mettre à jour les valeurs existantes
        QList<QLabel*> labels = widgetStats->findChildren<QLabel*>();

        if (labels.size() >= 6) {
            // Mettre à jour les pourcentages
            labels[0]->setText(QString("%1%").arg(pourcentPanne));
            labels[1]->setText(QString("%1%").arg(pourcentNeuf));
            labels[2]->setText(QString("%1%").arg(pourcentOccasion));

            // Mettre à jour les compteurs
            labels[3]->setText(QString("%1").arg(stats["En panne"]));
            labels[4]->setText(QString("%1").arg(stats["Neuf"]));
            labels[5]->setText(QString("%1").arg(stats["Occasion"]));
        }
        return; // Les stats sont déjà affichées
    }

    // 5. Créer le widget principal pour les stats
    widgetStats = new QWidget();
    widgetStats->setObjectName("widgetStatsEquipements");
    widgetStats->setStyleSheet(
        "QWidget {"
        "    background-color: #FFFFFF;"
        "    border: 2px solid #E0E6F0;"
        "    border-radius: 10px;"
    ...
    widgetStats->setFixedHeight(110);

    // 6. Layout principal
    QVBoxLayout *mainLayout = new QVBoxLayout(widgetStats);
    mainLayout->setContentsMargins(10, 10, 10, 10);
    mainLayout->setSpacing(5);

    // Titre
    QLabel *labelTitre = new QLabel("📊 STATISTIQUES DES ÉQUIPEMENTS");
    ...
    // 7. Layout pour les 3 statistiques
    ...
    QStringList categories = {"En panne", "Neuf", "Occasion"};
    QList<QColor> couleurs = {
        QColor(231, 76, 60),   // Rouge #E74C3C
        QColor(39, 174, 96),   // Vert #27AE60
        QColor(241, 196, 15)   // Jaune #F1C40F
    };
    QList<int> pourcentages = {pourcentPanne, pourcentNeuf, pourcentOccasion};
    QList<int> valeurs = {stats["En panne"], stats["Neuf"], stats["Occasion"]};
    ...
    for (int i = 0; i < 3; i++) {
        ...
        statWidget->setFixedSize(120, 80);

        QVBoxLayout *statLayout = new QVBoxLayout(statWidget);
        statLayout->setAlignment(Qt::AlignCenter);
        statLayout->setSpacing(2);

        // Le pourcentage (grand, comme un cercle)
        QLabel *labelPourcent = new QLabel(QString("%1%").arg(pourcentages[i]));
        labelPourcent->setObjectName("labelPourcent_" + categories[i].replace(" ", ""));
        labelPourcent->setAlignment(Qt::AlignCenter);
        labelPourcent->setStyleSheet(QString(
                                         "QLabel {"
                                         "    font-size: 28px;"
                                         "    font-weight: bold;"
                                         "    color: %1;"
                                         ...
                                         "    padding-top: 20px;"
                                         "}"
                                         ).arg(couleurs[i].name()).arg(couleurs[i].lighter(180).name()));

        // Le nom de la catégorie
        QLabel *labelCategorie = new QLabel(categories[i].toUpper());
        labelCategorie->setAlignment(Qt::AlignCenter);
        labelCategorie->setStyleSheet(
            "QLabel {"
            "    font-size: 11px;"
            "    font-weight: bold;"
            "    color: #7F8C8D;"
        ...
        QLabel *labelCount = new QLabel(QString("%1 équipements").arg(valeurs[i]));
        labelCount->setObjectName("labelCount_" + categories[i].replace(" ", ""));
        labelCount->setAlignment(Qt::AlignCenter);
        labelCount->setStyleSheet(
            "QLabel {"
            "    font-size: 10px;"
            "    color: #95A5A6;"
            "}"
        ...
        statLayout->addWidget(labelPourcent);
        statLayout->addWidget(labelCategorie);
        statLayout->addWidget(labelCount);
        statsLayout->addWidget(statWidget, 0, Qt::AlignCenter);
    }

    mainLayout->addLayout(statsLayout);

    // 8. Trouver où insérer les stats (au-dessus de la table)
    // Chercher le parent de la table
    QWidget *parentWidget = ui->equipementTable->parentWidget();

    if (parentWidget) {
        QVBoxLayout *parentLayout = qobject_cast<QVBoxLayout*>(parentWidget->layout());

        if (parentLayout) {
            // Insérer les stats en première position
            parentLayout->insertWidget(0, widgetStats);
        } else {
            // Si pas de layout, créer un nouveau layout
            QVBoxLayout *newLayout = new QVBoxLayout(parentWidget);
            newLayout->setContentsMargins(10, 10, 10, 10);
            newLayout->setSpacing(10);

            newLayout->addWidget(widgetStats);
            newLayout->addWidget(ui->equipementTable);
        }
    }
}

// Dans mainwindow.cpp - à la fin du fichier
void MainWindow::creerStatistiquesCirculairesEquipements()
{
    // Widget conteneur principal
    QWidget *statsWidget = new QWidget();
    statsWidget->setObjectName("statsWidgetEquipements");
    QVBoxLayout *mainStatsLayout = new QVBoxLayout(statsWidget);

    // Titre
    QLabel *titleLabel = new QLabel("📊 STATISTIQUES ÉQUIPEMENTS");
    titleLabel->setStyleSheet(
        "QLabel {"
        "    font-weight: bold;"
        "    font-size: 14px;"
    ...
    // Cercle En panne
    QLabel *circlePanne = new QLabel();
    circlePanne->setObjectName("circlePanne");
    circlePanne->setText("0%\nEN PANNE");
    circlePanne->setStyleSheet(circleStyle.arg("#E74C3C").arg("0.0").arg("#ECF0F1").arg("#E74C3C"));

    // Cercle Neuf
    QLabel *circleNeuf = new QLabel();
    circleNeuf->setObjectName("circleNeuf");
    circleNeuf->setText("0%\nNEUF");
    circleNeuf->setStyleSheet(circleStyle.arg("#27AE60").arg("0.0").arg("#ECF0F1").arg("#27AE60"));

    // Cercle Occasion
    QLabel *circleOccasion = new QLabel();
    circleOccasion->setObjectName("circleOccasion");
    circleOccasion->setText("0%\nOCCASION");
    circleOccasion->setStyleSheet(circleStyle.arg("#F39C12").arg("0.0").arg("#ECF0F1").arg("#F39C12"));

    // Ajouter les cercles
    circlesLayout->addWidget(circlePanne);
    circlesLayout->addWidget(circleNeuf);
    circlesLayout->addWidget(circleOccasion);
    ...
    QLabel *legendPanne = new QLabel();
    legendPanne->setObjectName("legendPanne");
    legendPanne->setText("🔴 0% En panne");
    legendPanne->setStyleSheet(legendStyle.arg("#E74C3C"));

    QLabel *legendNeuf = new QLabel();
    legendNeuf->setObjectName("legendNeuf");
    legendNeuf->setText("🟢 0% Neuf");
    legendNeuf->setStyleSheet(legendStyle.arg("#27AE60"));

    QLabel *legendOccasion = new QLabel();
    legendOccasion->setObjectName("legendOccasion");
    legendOccasion->setText("🟡 0% Occasion");
    legendOccasion->setStyleSheet(legendStyle.arg("#F39C12"));

    legendLayout->addWidget(legendPanne);
    legendLayout->addWidget(legendNeuf);
    legendLayout->addWidget(legendOccasion);
    legendLayout->setSpacing(5);

    mainStatsLayout->addWidget(legendWidget);

    // Ajouter à l'interface des équipements
    // Trouver où mettre les stats (au-dessus de la table)
    QWidget *parentWidget = ui->equipementTable->parentWidget();
    if (parentWidget) {
        QVBoxLayout *parentLayout = qobject_cast<QVBoxLayout*>(parentWidget->layout());
        if (!parentLayout) {
            parentLayout = new QVBoxLayout(parentWidget);
            parentLayout->setContentsMargins(10, 10, 10, 10);
            parentLayout->addWidget(ui->equipementTable);
        }
        parentLayout->insertWidget(0, statsWidget);
    }

    qDebug() << "✅ Statistiques circulaires équipements créées!";
}

void MainWindow::mettreAJourStatistiquesCirculairesEquipements()
{
    // Récupérer les statistiques
    QMap<QString, int> stats = Equipement::getStatistiquesParEtat();

    int panne = stats["En panne"];
    int neuf = stats["Neuf"];
    int occasion = stats["Occasion"];

    int total = panne + neuf + occasion;
    if (total == 0) total = 1; // Éviter division par zéro

    // Calculer les pourcentages
    double pannePercentage = (panne * 100.0) / total;
    double neufPercentage = (neuf * 100.0) / total;
    double occasionPercentage = (occasion * 100.0) / total;

    // Convertir en fractions pour les cercles
    double panneFraction = pannePercentage / 100.0;
    double neufFraction = neufPercentage / 100.0;
    double occasionFraction = occasionPercentage / 100.0;
    ...
    QLabel *circlePanne = findChild<QLabel*>("circlePanne");
    if (circlePanne) {
        circlePanne->setText(QString("%1%\nEN PANNE").arg(pannePercentage, 0, 'f', 0));
        circlePanne->setStyleSheet(circleStyle.arg("#E74C3C").arg(panneFraction).arg("#ECF0F1").arg("#E74C3C"));
    }

    QLabel *circleNeuf = findChild<QLabel*>("circleNeuf");
    if (circleNeuf) {
        circleNeuf->setText(QString("%1%\nNEUF").arg(neufPercentage, 0, 'f', 0));
        circleNeuf->setStyleSheet(circleStyle.arg("#27AE60").arg(neufFraction).arg("#ECF0F1").arg("#27AE60"));
    }

    QLabel *circleOccasion = findChild<QLabel*>("circleOccasion");
    if (circleOccasion) {
        circleOccasion->setText(QString("%1%\nOCCASION").arg(occasionPercentage, 0, 'f', 0));
        circleOccasion->setStyleSheet(circleStyle.arg("#F39C12").arg(occasionFraction).arg("#ECF0F1").arg("#F39C12"));
    }

    // Mettre à jour la légende
    QLabel *legendPanne = findChild<QLabel*>("legendPanne");
    QLabel *legendNeuf = findChild<QLabel*>("legendNeuf");
    QLabel *legendOccasion = findChild<QLabel*>("legendOccasion");

    if (legendPanne) legendPanne->setText(QString("🔴 %1% En panne").arg(pannePercentage, 0, 'f', 0));
    if (legendNeuf) legendNeuf->setText(QString("🟢 %1% Neuf").arg(neufPercentage, 0, 'f', 0));
    if (legendOccasion) legendOccasion->setText(QString("🟡 %1% Occasion").arg(occasionPercentage, 0, 'f', 0));

    qDebug() << "📊 Stats équipements mises à jour - Panne:" << pannePercentage << "%, Neuf:" << neufPercentage << "%, Occasion:" << occasionPercentage << "%";
}
